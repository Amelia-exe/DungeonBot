...
class HelpCommand(commands.HelpCommand):
    ...
    def get_cmd_string(self, cmd: commands.Command):
        params = [f"[{k}{' (optional)' if v.default != Parameter.empty else ''}]" for k, v in cmd.clean_params.items()]
    ...
    async def send_bot_help(self, mapping: Mapping[Optional[commands.Cog], List[commands.Command]]):
        ...
        for index, embed in enumerate(embeds, start=1):
            l = f"{embed.footer.text if embed.footer else ''}{embed.footer and ' | '}Page {index} of {len(embeds)}" 
            embed.set_footer(text=l, icon_url=self.context.bot.user.avatar_url)
